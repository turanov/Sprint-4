...
class ClientServiceTest {
    ...
    fun `fail save client - validation errors`() {
        ...
        assertEquals(exception.errorCode[0], ErrorCode.NOT_BE_EMPTY)
    }

    @Test
    fun `fail save client - validation errors of email`() {
    ...
    @Test
    fun `fail save client - validation errors of firstname`() {
    ...
    @Test
    fun `fail save client - validation errors of firstname long`() {
        val client = getClientFromJson("/fail/user_with_bad_firstname_too_long.json")
        val exception = assertFailsWith<ValidationException>("Ожидаемая ошибка") {
            clientService.saveClient(client)
        }
        assertEquals(exception.errorCode[0], ErrorCode.TOO_LONG)
    }

    @Test
    fun `fail save client - validation errors of lastname`() {
    ...
    @Test
    fun `fail save client - validation errors of snils`() {
